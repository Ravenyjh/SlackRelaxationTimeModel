...
def getGroups(a):
    ...
    for i in range(1, len(a)):
        if a[i][2] == init[2] and a[i][3] == init[3]:
            temp.append(a[i])
        else:
            group.append(temp)
            init = a[i]
...
def main(config):
    ...
    # 设置的dx偏分的点的差值
    for row in a:
        row[1], row[2], row[3] = mapCoordinate([row[1], row[2], row[3]], lattice_constants[0])
    a = sorted(a, key=lambda x: (x[2], x[3]))
    dx = a[1][1] - a[0][1]
    ...
    np.savetxt('SlackModelOutput/Slack.groupQEx',res,fmt="%.5f")
