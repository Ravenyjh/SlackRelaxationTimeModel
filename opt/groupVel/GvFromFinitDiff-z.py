...
def getGroups(a):
    ...
    for i in range(1, len(a)):
        if a[i][1] == init[1] and a[i][2] == init[2]:
            temp.append(a[i])
        else:
            group.append(temp)
            init = a[i]
...
def main(config):
    ...
    # 设置的dx偏分的点的差值
    for row in a:
        row[1], row[2], row[3] = mapCoordinate([row[1], row[2], row[3]], lattice_constants[2])
    ...
    np.savetxt('SlackModelOutput/Slack.groupQEz',res,fmt="%.5f")
